# Copied from https://github.com/grpc/grpc-java/blob/master/java_grpc_library.bzl
...
_gensource = rule(
    attrs = {
        ...
        "_java_plugin": attr.label(
            default = Label("@io_grpc_archive//compiler:grpc_java_plugin"),
            executable = True,
            cfg = "host",
        ),
    },
...
def java_grpc_library(name, srcs, deps, flavor=None,
  ...
  if flavor == "normal":
    ...
    added_deps += ["//3rdparty/jvm/io/grpc:grpc_protobuf_lite"]
  else:
